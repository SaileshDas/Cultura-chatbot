...
function App() {
	...
	const handleSubmit = async (e) => {
		...
		setInputMessage(''); 
		setResponseText('Awaiting response...');
		...
		try {
			...
			if (!response.ok) {
				setResponseText('Error: Could not reach the Cultura Backend server. Is it running on port 3001?');
				throw new Error(`HTTP error! status: ${response.status}`);
			}

			const data = await response.json();
			const chatbotResponseText = data.response || "I could not find a text response.";
			setResponseText(chatbotResponseText);
			...
			setIsTalking(false); 
			setResponseText(`Error: Failed to process request. Details: ${error.message}`);
		} 
	};
	// -------------------------
